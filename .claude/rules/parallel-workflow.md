...
- **`.claude/` 保護は依然有効**: `bypassPermissions` でも `.claude/settings.json` / `.claude/settings.local.json` / `.claude/rules/` / `.claude/CLAUDE.md` 等は承認プロンプトが出る。これら保護対象配下への書き込みが必要な場合は、事前に `permissions.allow` にパススコープ付き許可（例: `Edit(.claude/rules/**)`）を設定する（詳細は `.claude/rules/claude-dir-protection.md`）
