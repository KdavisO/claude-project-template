...
引数は空白区切りのトークン列として解釈し、次のいずれかのみを受け付けます:
...
- 複数指定可（例: `code pr`、`code docs issue`）
  - ただし `all` は**単独でのみ**指定可能です（`all` が含まれている場合は他トークンおよび `--team` 以外を同時に指定してはいけません）。
- `--team` は巡回対象指定と併用可能（例: `/patrol code pr --team`、`/patrol all --team`、`/patrol --team`）
