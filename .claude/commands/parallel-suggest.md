...
6. 各チームメイトに終了を指示してチームを解散する
   - **重要: チーム解散後も各PRのマージは手動で実行しないこと。** マージはポーリング（`/review-respond --auto --max-idle 3`）のポストアクションで自動実行される。Copilot再レビューが完了し、未対応コメントなしで`--max-idle`に達した場合にのみ安全にマージされる。再レビュー結果を待たずに手動マージすると、追加の指摘を見逃すリスクがある
