...
6. 各チームメイトに終了を指示してチームを解散する
   - **重要: チーム解散後も各PRのマージは手動で実行しないこと。** ポーリング（`/review-respond --auto --max-idle 3`）のポストアクションは、Copilot再レビューが完了し、未対応コメントがなく、`--max-idle` に達した場合にのみ安全に自動マージする。**Copilot再レビューが `requested_reviewers` に残っている間は自動マージはスキップされるため、その場合はマージされないままポーリングを継続するか、必要に応じて後でポーリングを再開すること。** 再レビュー結果を待たずに手動マージすると、追加の指摘を見逃すリスクがある
