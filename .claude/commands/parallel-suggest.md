...
4. 各チームメイトの進捗を監視し、実装・PR作成・ポーリング開始の完了報告を待つ
   - **ポーリング引き継ぎは行わない**。各チームメイトは `/issue-pr --auto` によって**自身のセッション**に `/loop ... /review-respond --auto --max-idle 3` を登録済みであり、cron発火時は**そのチームメイト自身のtmuxペイン**で `/review-respond` が実行される。これによりユーザーは各ペインでポーリング・コード修正・マージまでの進捗をリアルタイムに観測できる（Agent Teams + worktree 方式の最大の利点）
   - リードはチームメイトの完了報告（実装・PR作成・ポーリング開始まで）を受け取ったら、**チームメイトをシャットダウンせずアイドル状態で生存させたままにする**。チームメイトが自走してポーリング→自動マージ→worktreeクリーンアップを完結させるまで介入しない
5. **全チームメイト自走終了確認 → 解散**:
   - 各チームメイトのポーリングが `--max-idle 3` で自然停止し、ポストアクション（PRマージ・worktreeクリーンアップ・ステータスファイル削除）まで完結したことを確認する
   - 確認方法:
     - `gh pr view {PR番号} --json state,mergedAt` でPRがマージ済みか確認
     - `/tmp/{project}-flow-{ownerRepo}-{issue番号}` のステータスファイルが削除されているか確認（チームメイトのテンプレート手順8の正常終了時削除が発火していれば存在しない）
...
6. PR作成・自動レビューフロー:
   ...
   - PRマージは `/review-respond --auto --max-idle 3` のポストアクション（手順10-A）で自動実行されるが、このポストアクション自体は `--max-idle` による自動停止が発動し、停止理由が「未対応コメントなし」の場合にのみ実行される。Copilotレビュー未着で停止した場合などは自動マージが走らない。worktreeクリーンアップ（手順10-C）はポーリング実行時のCWDがworktree内である場合のみ自動実行される。ポーリングは**本チームメイト自身のセッション**（CWDはworktree内）で発火するため、通常は自動クリーンアップされる想定

7. 実装・PR作成・ポーリング開始完了をリードに報告（ブランチ名、PR番号、worktreeの絶対パス、変更ファイル一覧を含める）
   - 報告後も**シャットダウンせずアイドル状態で生存したままにすること**。ポーリング（`/loop ... /review-respond --auto --max-idle 3`）は本チームメイト自身のセッションに登録されており、cron発火時に**このセッション自身のtmuxペインで** `/review-respond` が実行される。リードがユーザーの進捗をリアルタイムに観測できるよう、ポーリングが `--max-idle 3` で自然停止するまで生存する

8. ポーリング完了後のステータスファイルのクリーンアップ:
   - ポーリングが `--max-idle 3` で自然停止するまで待機してから、以下を実行する
   - `/review-respond --auto` のポストアクション（PRマージ・worktreeクリーンアップ）が完了した後、以下のクリーンアップを実施する
   - 正常終了時:
