...
5. **全チームメイト自走終了確認 → 解散**:
   ...
   - あわせて、チームメイトが下部テンプレートの手順8（ステータスファイル削除）を実施済み、または異常終了時の運用に従ってステータスファイルを保持した状態になっているかを確認する
   - 確認方法:
     - `gh pr view {PR番号} --json state,mergedAt` でPRがマージ済みか確認
     - `git worktree list` で対象worktreeが削除されているか確認
     - `/tmp/{project}-flow-{ownerRepo}-{issue番号}` のステータスファイルが**存在しない**場合は、チームメイトのテンプレート手順8によるクリーンアップ実施済みと判断してよい
