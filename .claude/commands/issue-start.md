...
`/suggest-next` の実行後、**`--continuous` フラグの有無に応じて必ず以下のように分岐する**:

- **`--continuous` なし**: `/suggest-next` の結果を表示して終了。これ以上の処理は行わない。
- **`--continuous` あり**: `/suggest-next` の結果を表示した後、**必ず「連続実行フロー」セクションの手順に進む**（ここで停止してはならない）。`/suggest-next` の出力結果は連続実行フローの次Issue選定に使用する。
...
前セクション「次のIssue提案とフロー遷移」で `/suggest-next` は実行済みである。その結果を使い、以下の手順で次のIssueを選定・着手する（`/suggest-next` を再実行しないこと）:
