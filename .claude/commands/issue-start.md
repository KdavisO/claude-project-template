...
2. **除外フィルタを適用**（以下に該当するIssueを候補から除外する）:
   ...
   - **オープンPRと変更ファイルが競合するIssue**: 以下の手順で判定
     1. `gh pr list --state open --json number,headRefName --limit 20` でオープンPR一覧を取得
     ...
     3. 候補Issueのタイトル・本文に含まれるファイルパスやコンポーネント名がPR変更ファイルと重複する場合に除外
        ...
        - ただし、Issue本文にファイルパス情報がない場合はこのフィルタをスキップ（除外しない）
...
4. **上位1〜3件を提案として出力**:
   ```
   --- 次のIssue候補 ---
   1. #{issue番号} {タイトル}（優先度: {high/medium/low/なし}）
...
- 提案処理でエラーが発生した場合（GitHub API失敗等）は、エラーを表示せず提案セクション自体をスキップする（フロー全体を失敗させない）
- 調査モード（`research` ラベル）のフロー完了後も提案を実行する
- 自動モードのポストアクション内で実行する場合は、完了サマリーの直後に出力する
