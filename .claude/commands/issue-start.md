...
- `--parallel` / `--auto` / `--continuous` / `--max-issues` フラグは調査モードではすべて無視される（調査モードのフローが優先）。共通手順1のフラグ検証（`--parallel` なしの `--auto` はエラー、`--continuous` なしの `--max-issues` はエラー等）も、`research` ラベルが検知された場合はスキップする
