...
- `--no-skills`（例: `256 --parallel --auto --no-skills`）→ スキル自動適用をスキップし、従来の直接実装フローを使用（`--parallel --auto` と併用）
...
**重要: `--auto` モードではポストアクションを含む全手順（手順1〜7）において、ユーザーへの確認・方針提案で停止してはならない。各ステップを自律的に判断・実行し、中断なしで完了すること。スコープ外Issue候補の作成も自動実行し、作成されたIssueのURLをログとして出力する。**
...
`--no-skills` フラグが指定されていない場合、自動モードのフロー手順2（実装前分析）で以下のスキル自動選択・実行を行う。`--no-skills` が指定されている場合はこのセクションをスキップし、従来の直接実装フローを使用する。
...
セルフレビュー・コミット後、PR作成前に `/requesting-code-review` を実行するかを以下の基準で判定する:
...
4. **セルフレビュー・コミット**:
   ...
   - 適切な粒度でコミット
   - **PR作成前コードレビュー**: `--no-skills` 未指定時、「スキルオーケストレーション」セクションの「PR作成前コードレビュー」基準に従い、該当する場合は `/requesting-code-review` を実行する。Critical指摘があれば修正・再コミットする
5. **PR作成・レビュー対応ポーリング開始**:
...
6. **ポストアクション**（停止理由が「未対応コメントなし」の `--max-idle` 自動停止時のみ自動実行）:
...
7. **完了サマリーを出力する**（ポストアクション完了後、またはポーリング停止後に出力。エラー停止時は出力しない）:
   ```
   === 完了サマリー ===
   Issue: #{issue番号} {Issueタイトル}
   ブランチ: {ブランチ名}
   ...
   ==================
   ```
   - 変更ファイル・コミット一覧は手順6のポストアクション冒頭で事前取得済みのデータを使用する
