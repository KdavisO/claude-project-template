...
4. **PR作成・レビュー対応ポーリング開始**:
   ...
   - `/issue-pr --auto` がPR作成後、Copilotレビューリクエストが成功した場合のみ自動で `/loop 5m --skip-first /review-respond --auto --max-idle 3` を開始する（`--team` 指定時は `/issue-pr --auto --team` を実行し、ポーリングコマンドが `/loop 5m --skip-first /review-respond --auto --team --max-idle 3` となる）。レビューリクエストが失敗した場合はポーリングを開始せず、警告を出力して終了する（`phase: "polling"` への更新も行わない）
