...
4. **PR作成・レビュー対応ポーリング開始**:
   ...
   - `/issue-pr --auto` がPR作成後に自動で `/loop 5m --skip-first /review-respond --auto --max-idle 3` を開始する（`--team` 指定時は `/issue-pr --auto --team` を実行し、ポーリングコマンドが `/loop 5m --skip-first /review-respond --auto --team --max-idle 3` となる）
