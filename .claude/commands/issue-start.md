...
6. **完了サマリーを出力する**（ポストアクション完了後、またはポーリング停止後に必ず出力）:
   ```
   === 完了サマリー ===
   Issue: #{issue番号} {Issueタイトル}
   ブランチ: {ブランチ名}
   ...
   ==================
   ```
   - 変更ファイル一覧は `git diff --name-only origin/main...{ブランチ名}` で取得する（worktree削除前に取得しておくこと）
   - コミット一覧は `git log --oneline origin/main..{ブランチ名}` で取得する（worktree削除前に取得しておくこと）
   - ポストアクションが実行されなかった場合（Copilotレビュー未着での停止等）も、マージ欄を「スキップ」として出力する
