...
自動モードでは各フェーズの遷移時にステータスファイル（`/tmp/{project}-flow-{issue番号}`）を更新する。これにより `/flow-status` コマンドで進捗を確認できる。
...
- エラー停止時: `phase` を `"error"` に更新し、`error` フィールドにエラー内容を追記する。**エラー時はステータスファイルを削除しない**（`/flow-status` でエラー状態を確認できるようにするため）。ユーザーが手動で対処した後、ファイルを手動削除するか `/worktree-cleanup` で一括削除する
...
4. **PR作成・レビュー対応ポーリング開始**:
   - ステータス更新: `phase: "pr-created"`（PR番号も `pr` フィールドに設定）
