...
1. `settings.json` に `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` が設定済み（テンプレートに含まれる）
2. 無効化する場合は `settings.json` の `env` セクションから該当行を削除

### カスタマイズ

- **チームメイトの役割**: `.claude/rules/agent-teams.md` のチームメイト作成例を編集
- **レビュー分担の閾値**: `.claude/commands/review-respond.md` の「5件以上」の閾値を調整
- **表示モード**: `settings.json` に `"teammateMode": "auto"` / `"in-process"` / `"tmux"` を追加
